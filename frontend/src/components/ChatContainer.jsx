  ...
  useEffect(() => {
    if (messages && messages.length > 0) {
      ...
      messages.forEach(message => {
        if (!message.isDeleted && message.images && Array.isArray(message.images) && message.images.length > 0) {
      ...
      >
        {messages.map((message, index) => (
          ...
          >
            ...
            <div className={`chat-bubble flex flex-col overflow-hidden ${messageStyles.transition} ${message.isDeleted ? messageStyles.deleted : messageStyles.normal}`}>
              {message.isDeleted ? (
                    ...
                    message.images.map((img, index) => (
                      <div key={index} className="mb-2 relative">
                        ...
                        <div 
                          ...
                          onClick={() => openImageModal(img)}
                        >
                          <span className="text-white opacity-0 hover:opacity-100">Xem</span>
                        </div>
                      </div>
                    ))}
      ...
      {confirmDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-base-100 rounded-lg p-6 max-w-sm w-full shadow-xl animate-scaleIn">
            ...
            <p className="py-2">Bạn có chắc chắn muốn thu hồi tin nhắn này không?</p>
            <p className="text-sm opacity-70 mt-1 mb-4">Tin nhắn sẽ bị thu hồi cho tất cả mọi người.</p>
